...
static void Bsp_system_task(void* parameter)
{
	...
	rt_tick_t currentTicks = 0;

	//获取系统复位的时间
	currentTicks = rt_tick_get();
	...
	for(;;)
	{
		...
		if(TimingDelay % 5 == 0)
		{
//            pKW_SHM->ds.dc5v_detect = GetPin(DC5V_DETECT);
//            pKW_SHM->ds.dc110v_detect = GetPin(DC110V_DETECT);

//            if(pKW_SHM->ds.dc5v_detect != PIN_HIGH && pKW_SHM->ds.dc5v_detect != PIN_HIGH)
//            {
//                System_Led_SetMode(LED_FLT, MODE_FLASH_SLOW);
//                rt_kprintf("+ power detect err!\r\n");
//            }
//            
//            flt_clear();
