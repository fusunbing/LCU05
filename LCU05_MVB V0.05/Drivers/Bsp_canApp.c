...
static uint32_t can1_cnt[CAR_NUM] = { 0 };
static uint32_t can2_cnt[CAR_NUM] = { 0 };

static void CanNode_Update(void)
{
    uint32_t i;
    ...
    for(i = 0; i < CAR_NUM; i++)
    {
        ...
        ds.car[i].can1 = (can1_cnt[i] > FLT_TIMEOUT) ? RT_ERROR : RT_EOK;
        ds.car[i].can2 = (can2_cnt[i] > FLT_TIMEOUT) ? RT_ERROR : RT_EOK;
        ds.car[i].lost = (ds.car[i].can1 == RT_ERROR && ds.car[i].can2 == RT_ERROR) ? RT_ERROR : RT_EOK;
        
        if(ds.car[i].lost == RT_ERROR)
        {
            ds.mvb_port[i + 2].data[0] |= 0x04;     //严重故障
            ds.mvb_port[i + 2].data[5] |= 0x10;     //CAN网关板前面板CAN1故障
            ds.mvb_port[i + 2].data[5] |= 0x20;     //CAN网关板前面板CAN2故障
            ds.mvb_port[i + 2].data[4] |= 0x10;     //设备丢失
        }
    }
}


static void CanNode_Clear(uint32_t port, uint32_t carID)
{
    if(carID < CAR_NUM)
    {
        switch(port)
