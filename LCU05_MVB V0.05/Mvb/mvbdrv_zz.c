...
//寻址UART emulation
//#define 	Uart_RD(reg) *((u8*)((UART_ABASE+reg)<<1))
//#define 	Uart_WR(reg,val) (*(u8*)((UART_ABASE + reg)<<1) = val)
...
u8	ClearMvb(void)
{
    ...
    /*	for(;;) 
    {
    temp = Uart_RD( UART_TEST);
    temp = Uart_RD( UART_LSR);
    temp = Uart_RD( UART_THR);
    Uart_WR( UART_TEST,0x55);
    }
    */		
    ...
    if(result == RET_OK)	
    {
        ...
        if(result == RET_OK)
        {
            ...
            if(BusStatus != 0)
            {
                ...
                for(i=0; i<Len; i++)//	Len//for(i=0; i<32; i++)//	Len					
                {
                    *PointerReceive++ = ReceiveMvb();
                }

//                for(i=0; i<MAXMVBBYTE-Len; i++)	ReceiveMvb();
...
u8	StopMvb()
{
    ...
    /*
    delay_ms(1);								//wait for appromixly 250us
    MVBDAddr = 0x0000;							//clear 
    result = MVB_check_THRE_ON();
    if(result == TRUE)
    {
    TransmiteMvb('C');
