 ...
 *
 * $Id: vos_sock.c 1748 2018-07-13 15:59:36Z bloehr $*
 *
...
{
    BOOL8   found = FALSE;
    /*20180811 == only one netif for LCU03 ETH Board*/
    found = (BOOL8)eth_device_getMacAddress(pMacAddr);	
    ...
    const VOS_SOCK_OPT_T    *pOptions)
{
    ...
    if (pOptions)
    {
        ...
        {
            if (lwip_fcntl(sock, F_SETFL, O_NONBLOCK) == -1)
            {
                char buff[VOS_MAX_ERR_STR_SIZE];
                STRING_ERR(buff);
    ...
    UINT16      port)
{
    ...
    *pSize  = 0;
    ...
    rt_memset(&destAddr, 0, sizeof(destAddr));
    ...
    UINT16  port)
{
    ...
    rt_memset((char *)&srcAddress, 0, sizeof(srcAddress));
    ...
    UINT32      *pSize)
{
    ...
    *pSize      = 0;
    ...
    do
    {
        sendSize = lwip_write(sock, (char *)pBuffer, bufferSize);
        if (sendSize >= 0)
        {
            bufferSize  -= sendSize;
            pBuffer     += sendSize;
    ...
    UINT32  *pSize)
{
    ...
    *pSize = 0;
    ...
    do
    {
        rcvSize = lwip_read(sock, (char *)pBuffer, bufferSize);
        if (rcvSize > 0)
        {
            bufferSize  -= rcvSize;
            pBuffer     += rcvSize;
