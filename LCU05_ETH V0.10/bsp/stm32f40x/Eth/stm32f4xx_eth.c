...
/**
  * @brief  Clears the ETHERNET隆炉s DMA pending flag.
...
/**
  * @brief  Clears the ETHERNET隆炉s DMA IT pending bit.
...
/* config Addr */
uint8_t ConfigEthDevice(uint32_t ipaddr, uint32_t gateway, uint32_t netmask)
{
	ip_addr_t  ip;
	ip_addr_t  gw;
	ip_addr_t  mk;
	...
	netif_set_ipaddr(stm32_eth_device.parent.netif,&ip);
	netif_set_gw(stm32_eth_device.parent.netif,&gw);
	netif_set_netmask(stm32_eth_device.parent.netif,&mk);
	return 1;
}
...
//    while (ETH_GetSoftwareResetStatus() == SET);
    ...
    //这是完全屏蔽，就是因为这个，我收不到组播数据。
    //将其改为ETH_MulticastFramesFilter_None不屏蔽，这样就收到组播数据了。
    ETH_InitStructure.ETH_MulticastFramesFilter = ETH_MulticastFramesFilter_None;//开启组播模式
    ...
    ETH_InitStructure.ETH_RxDMABurstLength = ETH_RxDMABurstLength_4xPBL_128Beat;
    //ETH_InitStructure.ETH_TxDMABurstLength = ETH_TxDMABurstLength_32Beat;
		ETH_InitStructure.ETH_TxDMABurstLength = ETH_TxDMABurstLength_4xPBL_128Beat;
