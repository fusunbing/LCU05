...
void SD_LowLevel_DeInit(void)
{
  ...
  GPIO_PinAFConfig(GPIOB, GPIO_PinSource2, GPIO_AF_MCO);
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource8, GPIO_AF_MCO);
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_MCO);
  GPIO_PinAFConfig(GPIOD, GPIO_PinSource2, GPIO_AF_MCO);

  /* Configure PC.08, PC.11 pins: D2 pins */
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  else if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    ...
    cardinfo->CardCapacity = (uint64_t)(((uint64_t)cardinfo->SD_csd.DeviceSize + 1) * 512 * 1024);
...
/*	NVIC_InitTypeDef NVIC_InitStructure;
	...
	{
		RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("init sd lock semaphore failed\n"));
	}
	else
		RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("SD Card init OK\n"));
...
static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
	...
	RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("sd: read 0x%X, sector 0x%X, 0x%X\n", (uint32_t)buffer ,pos, size));
	...
	RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("read failed: %d, buffer 0x%08x\n", status, buffer));
	return 0;
}
...
static rt_size_t rt_sdcard_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	...
	//RT_DEBUG_LOG(RT_SD_DEBUG_EN, "sd: write 0x%X, sector 0x%X, 0x%X\n", (uint32_t)buffer , pos, size);
	rt_sem_take(&sd_lock, RT_WAITING_FOREVER);
	...
	RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("write failed: %d, buffer 0x%08x\n", status, buffer));
	return 0;
}

static rt_err_t rt_sdcard_control(rt_device_t dev, uint8_t cmd, void *args)
{
...
void rt_hw_sdcard_init(void)
{
	...
	if (SD_Init() == SD_OK)
	{
		...
		// if (sector == RT_NULL)
		// {
			// RT_DEBUG_LOG(RT_SD_DEBUG_EN, "allocate partition sector buffer failed\n");
			// return;
		// }
...
__return:
	RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("sdcard init failed\n"));
}
...
void SDIO_IRQHandler(void)
{
    ...
    if( SD_ProcessIRQSrc() == 2)
		RT_DEBUG_LOG(RT_SD_DEBUG_EN, ("SD Error\n"));
