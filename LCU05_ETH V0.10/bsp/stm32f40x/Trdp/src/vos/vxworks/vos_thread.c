 ...
 *
 * $Id: vos_thread.c 1749 2018-07-19 16:38:21Z bloehr $*
 *
    ...
    VOS_TIMEVAL_T *pTime)
{
	rt_tick_t tick=0;
	tick = rt_tick_get();
	pTime->tv_sec   = tick/1000;
    pTime->tv_usec  = (INT32) (tick%1000*1000);
...
{
		VOS_ERR_T result = VOS_NO_ERR;
	  *pMutex = rt_mutex_create("vos_mutex", RT_IPC_FLAG_FIFO);	
		return result;
...
//    else
//    {
//        *pMutex = (VOS_MUTEX_T) vos_memAlloc(sizeof(VOS_MUTEX));
...
{
	VOS_ERR_T result = VOS_NO_ERR;
	rt_mutex_init (pMutex, "vos_mutex", RT_IPC_FLAG_FIFO);
	return result;
...
{
    VOS_ERR_T result = VOS_NO_ERR; 
	rt_err_t rt_result = -1;
	rt_result=rt_mutex_take(pMutex,RT_WAITING_FOREVER);
	return result;
...
{
	VOS_ERR_T result = VOS_NO_ERR;
	rt_kprintf("vos_mutexTryLock not instantiation\r\n");
	return result;
...
{
	VOS_ERR_T result = VOS_NO_ERR;
	rt_kprintf("vos_semaCreate not instantiation\r\n");
	return result;
...
{
	VOS_ERR_T result = VOS_NO_ERR;
	rt_kprintf("vos_semaTake not instantiation\r\n");
	return result;
